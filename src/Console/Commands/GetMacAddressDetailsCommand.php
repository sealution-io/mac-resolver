...
use Sealution\MacResolver\Exceptions\InvalidMacAddressFormatException;
...
class GetMacAddressDetailsCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $oui = self::getOUI($mac_address);

        $vendor = self::getMacAddressDetails($oui);

        if ($vendor == null)
            $this->error("Cannot Resolve Mac Address");
